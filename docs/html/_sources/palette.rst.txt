...
The function getColorsW3C can be used not only to extract the percentage of colors accordingly to the W3C specification, but can also be used to generate a palette of n colors from an image.
...
>>> import pysiology
>>> sampleImg = "path/to/logo"
>>> img = cv2.imread(sampleImg, cv2.IMREAD_UNCHANGED)
>>> img = cv2.cvtColor(img, cv2.COLOR_BGRA2RGBA)
>>> #Generate a palette of 5 colors using the 140 W3C specification and plot the results
>>> pyaeshtetics.colorDetection.getColorsW3C(img, ncolors=140, plot=True, plotncolors=5)
